...
function Command({ className, ...props }: React.ComponentProps<typeof CommandPrimitive>) {
  return (
    <CommandPrimitive
      className={cn(
        'flex h-full w-full flex-col overflow-hidden rounded-md bg-hw-surface text-hw-text',
        className,
      )}
      {...props}
    />
  ...
  return (
    <div className="flex items-center border-hw-border border-b px-3" cmdk-input-wrapper="" data-slot="command-input">
      ...
      <CommandPrimitive.Input
        className={cn(
          'flex h-11 w-full rounded-md bg-transparent py-3 text-sm outline-hidden text-hw-text placeholder:text-hw-text-secondary disabled:cursor-not-allowed disabled:opacity-50',
          className,
        )}
        {...props}
      />
