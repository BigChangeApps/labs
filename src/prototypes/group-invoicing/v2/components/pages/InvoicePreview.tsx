...
interface InvoiceData {
  ...
  invoiceNumberPrefix: string; // e.g., "IV" or "HS"
  name: string;
  address: string;
  jobs: JobWithLines[];
  ...
  logo?: string; // Base64 or URL of the logo image
    ...
    <div className="relative size-[18px]">
      <div className="absolute inset-0 rounded-full bg-white shadow-[0px_0px_0px_1px_rgba(3,7,18,0.08),0px_0.45px_1.8px_0px_rgba(11,38,66,0.16)] overflow-hidden">
        <div className="absolute inset-[5%] rounded-full bg-hw-surface-subtle flex items-center justify-center">
          <span className="text-[8px] font-semibold text-hw-text-secondary leading-none">{initials}</span>
        </div>
      </div>
    </div>
  );
  ...
  if (isInactive) {
    return (
      <div className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-[rgba(26,28,46,0.05)] border border-hw-border w-fit">
        <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">{total} lines</span>
      </div>
    );
  }
  return (
...
function JobTypeBadge({ type }: { type: string }) {
  return (
    <div className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-white border border-hw-border">
      <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">{type}</span>
    </div>
  );
}
      ...
      <div className="flex items-center justify-between px-4 py-3">
        <div className="flex flex-col gap-1.5 flex-1">
          <div className="flex items-center gap-1.5">
            <span className="text-sm font-bold text-hw-text tracking-[-0.14px]">{job.jobRef}</span>
            <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">{job.completed}</span>
            <ResourceAvatar initials={job.jobCategory === "Internal" ? "CS" : "LB"} />
          </div>
          {fieldValues.length > 0 && (
            <div className="flex flex-col gap-0.5 mt-0.5">
              {fieldValues.map(({ label, value }) => (
                <span key={label} className="text-xs text-hw-text-secondary tracking-[-0.14px]">
                  {label}: {value}
                </span>
              ))}
            </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm font-bold text-hw-text tracking-[-0.14px]">{formatCurrency(job.leftToInvoice)}</span>
        </div>
      </div>
    </div>
  );
...
function SummaryJobView({ 
  ...
  return (
    <div className="space-y-4">
      {/* Service heading */}
      <h3 className="text-sm font-bold text-hw-text tracking-[-0.14px]">
      ...
      <div className="bg-white rounded-lg border border-hw-border overflow-hidden">
        <div className="flex items-center justify-between pl-4 pr-3 py-3">
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-1.5">
              <span className="text-sm font-bold text-hw-text tracking-[-0.14px]">{firstJob?.jobRef || "INT/12345"}</span>
          ...
          <div className="flex items-center">
            <span className="text-sm font-bold text-hw-text tracking-[-0.14px]">{formatCurrency(totalValue)}</span>
          </div>
        </div>
      </div>
    </div>
...
function DetailedJobView({
  ...
  return (
    <div className="border border-hw-border rounded-lg overflow-hidden">
      {/* Table Header */}
      <div className="flex items-center gap-5 px-3 py-2 bg-muted border-b border-hw-border">
        <span className="flex-1 text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Name</span>
        <span className="w-24 text-sm font-medium text-hw-text-secondary tracking-[-0.14px] text-right">Unit price</span>
        <span className="w-24 text-sm font-medium text-hw-text-secondary tracking-[-0.14px] text-right">Total</span>
      ...
      <div className="divide-y divide-[rgba(26,28,46,0.08)]">
        {lineItems.map((item) => {
          ...
          return (
            <div key={item.id} className="flex items-center gap-5 px-3 py-2 bg-white hover:bg-hw-surface-subtle transition-colors min-h-[40px] max-h-[56px]">
              <div className="flex-1 flex items-center gap-2.5">
                <CategoryDot category={category} />
                <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{item.jobRef}</span>
              </div>
              <span className="w-24 text-sm font-medium text-hw-text tracking-[-0.14px] text-right">{formatCurrency(item.unitPrice)}</span>
              <span className="w-24 text-sm font-medium text-hw-text tracking-[-0.14px] text-right">{formatCurrency(item.total)}</span>
              <button 
                onClick={() => onEditLineItem?.(item)}
                className="p-1 hover:bg-[rgba(11,38,66,0.08)] rounded transition-colors"
                aria-label="Edit line item"
              >
                <Pencil className="h-4 w-4 text-hw-text-secondary" />
              </button>
            </div>
          );
        })}
...
function GroupJobCardPreview({ 
  ...
  return (
    <div className="bg-white rounded-lg border border-hw-border overflow-hidden">
      <div className="bg-hw-surface-subtle border-b border-hw-border px-3 py-3">
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-1.5 flex-1">
            <div className="flex items-center gap-2">
              <StacksIcon />
              <span className="text-sm font-bold text-hw-text tracking-[-0.14px]">{groupJob.jobRef}</span>
              <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">21 May - 3 June 2025</span>
            </div>
      ...
      <div className="p-4 space-y-3">
        ...
        {(groupLinesSelected || !showPartial) && (
          <div className="bg-white rounded-lg border border-hw-border overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-3">
              <div className="flex-1 flex flex-col gap-1.5">
                <span className="text-sm font-bold text-hw-text tracking-[-0.14px]">Group-level lines</span>
                <LinesBadge total={10} />
              </div>
              <span className={cn(
                "text-sm font-bold tracking-[-0.14px]",
                groupLinesSelected || !showPartial ? "text-hw-text" : "text-hw-inactive line-through"
              )}>
      ...
      {attachments.length > 0 && (
        <div className="space-y-1.5">
          {attachments.map((attachment) => (
            <div
              key={attachment.id}
              className="flex items-center justify-between px-3 py-2 bg-hw-surface-subtle rounded-md"
            >
...
function DatePicker({
  ...
  return (
    <div className="space-y-1.5">
      <label className="text-sm font-medium text-hw-text tracking-[-0.14px]">{label}</label>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            className={cn(
              "flex items-center justify-between w-full h-8 px-2 bg-white rounded-md shadow-[0px_0px_0px_1px_rgba(3,7,18,0.08),0px_0.5px_2px_0px_rgba(11,38,66,0.16)] hover:bg-hw-surface-hover transition-colors text-left",
              !date && "text-hw-inactive"
            )}
          >
            <span className={cn(
              "text-sm tracking-[-0.14px]",
              date ? "text-hw-text" : "text-hw-inactive"
            )}>
              {date ? format(date, "dd/MM/yyyy") : "DD/MM/YYYY"}
            </span>
            <CalendarIcon className="h-5 w-5 text-hw-text" />
          </button>
        </PopoverTrigger>
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <div className="relative w-full">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-hw-text-secondary pointer-events-none z-10" />
          <Input
            ref={inputRef}
            type="text"
          ...
          />
          <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-hw-text-secondary pointer-events-none z-10" />
        </div>
      </PopoverTrigger>
      <PopoverContent
    ...
    <div className="h-screen bg-muted flex flex-col overflow-hidden">
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-white border-b border-hw-border shrink-0">
        <div className="flex items-center gap-2 text-sm">
          <span className="text-hw-text-secondary">Jobs ready to invoice</span>
        ...
        <Button 
          variant="secondary" 
          size="sm"
          onClick={() => navigate("/group-invoicing/v2/create")}
        >
          Save as draft
        </Button>
      </header>
        ...
        <div className="bg-white border-r border-hw-border overflow-auto">
          <div className="px-[46px] py-[32px] space-y-10 w-[780px]">
            ...
            <div className="flex items-center justify-between">
              <button 
                onClick={() => navigate("/group-invoicing/v2/create")}
                className="flex items-center gap-1.5 text-sm font-medium text-hw-text-secondary hover:underline"
              >
              ...
              {totalInvoiceCount > 1 && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-1 rounded-md bg-white shadow-[0px_0px_0px_1px_rgba(11,38,66,0.08)] hover:bg-hw-surface-subtle transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    aria-label="Previous invoice"
                  >
                    <ChevronLeft className="h-6 w-6 text-hw-text" />
                  </button>
                  <span className="text-base text-hw-text">
                    <span className="font-medium">{currentInvoiceIndex + 1}</span>
                    <span className="text-hw-text-secondary"> of {totalInvoiceCount}</span>
                  </span>
                  <button
                    onClick={handleNext}
                  ...
                  >
                    <ChevronRight className="h-6 w-6 text-hw-text" />
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-hw-text tracking-[-0.14px]">Invoice title</label>
              <Input
                type="text"
            ...
            <div className="space-y-8">
              ...
              <div className="flex gap-[114px]">
                ...
                <div className="w-[302px] space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-hw-text tracking-[-0.14px]">Invoice number</label>
                    <Input
                      type="text"
                      value={`${invoiceData.invoiceNumberPrefix || "IV"}/${invoiceData.invoiceNumber.toString().padStart(4, "0")}`}
                      onChange={(e) => {
                        const value = e.target.value;
                        ...
                        if (match) {
                          setInvoiceData((prev) => ({
                            ...
                            invoiceNumber: parseInt(match[2], 10) || prev.invoiceNumber,
                          }));
                        }
                      }}
                      className="h-8"
                  ...
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-hw-text tracking-[-0.14px]">Contact</label>
                    <Input
                      type="text"
              ...
              <div className="w-[302px] space-y-1.5">
                <label className="text-sm font-medium text-hw-text tracking-[-0.14px]">Reference</label>
                <Input
                  type="text"
              ...
              <div className="w-[302px] space-y-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-hw-text tracking-[-0.14px]">Nominal code</label>
                  <Input type="text" value="5001" className="h-8 text-hw-text-secondary" readOnly />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-hw-text tracking-[-0.14px]">Department code</label>
                  <Input type="text" value="67800" className="h-8 text-hw-text-secondary" readOnly />
                </div>
              </div>
              ...
              <div className="w-[302px] space-y-2">
                <label className="text-sm font-medium text-hw-text tracking-[-0.14px]">
                  Line level detail
                </label>
                <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-[7px]">
                      {selectedLineDetailFields.map((fieldKey) => {
                        ...
                        return (
                          <div
                            key={fieldKey}
                            className="inline-flex items-center gap-1 px-1 h-5 bg-hw-surface-subtle rounded-md"
                          >
                            <span className="text-xs text-hw-text-secondary tracking-[-0.12px]">{field.label}</span>
                            <button
                              onClick={() => handleRemoveField(fieldKey)}
                              className="hover:bg-[rgba(11,38,66,0.08)] rounded transition-colors"
                            >
                              <X className="h-3 w-3 text-hw-text" />
                            </button>
                          </div>
                        );
                      })}
              ...
              <div className="w-[302px] space-y-6">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-hw-text tracking-[-0.14px]">Notes</label>
                  <textarea
                    value={invoiceData.notes}
                    onChange={(e) => setInvoiceData((prev) => ({ ...prev, notes: e.target.value }))}
                    className="w-full h-[66px] px-2 py-2.5 bg-white rounded-md shadow-[0px_0px_0px_1px_rgba(3,7,18,0.08),0px_0.5px_2px_0px_rgba(11,38,66,0.16)] resize-none text-sm text-hw-text placeholder:text-hw-inactive focus:outline-none focus:ring-1 focus:ring-[#086DFF]"
              ...
              <div className="flex items-center gap-1">
                <Building2 className="h-6 w-6 text-hw-text-secondary" />
                        ...
                        <div className="flex items-center gap-2">
                          <div className={cn(
                            ...
                            <span className={cn(
                              ...
                              isJobSelected ? "text-hw-brand" : "text-hw-text-secondary"
                            )}>
                          ...
                          {job.jobCategory && (
                            <div className="inline-flex items-center px-1.5 py-px h-5 rounded-md bg-white border border-hw-border">
                              <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">{job.jobCategory}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      <div className="text-sm font-bold text-hw-text tracking-[-0.14px]">
                        {formatCurrency(job.leftToInvoice)}
                      </div>
                    </div>
                  );
        ...
        <div className="flex-1 bg-[#F9FAFD] overflow-auto flex flex-col items-center py-8 gap-4">
          ...
          <div className="w-[592px] flex items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <button className="inline-flex items-center gap-1 h-7 px-2 bg-white rounded-md shadow-[0px_0px_0px_1px_rgba(11,38,66,0.08)] hover:bg-hw-surface-subtle transition-colors">
                <Download className="h-4 w-4 text-[#0A0A0A]" />
                <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">PDF</span>
              </button>
              <button className="inline-flex items-center gap-1 h-7 px-2 bg-white rounded-md shadow-[0px_0px_0px_1px_rgba(11,38,66,0.08)] hover:bg-hw-surface-subtle transition-colors">
                <Printer className="h-4 w-4 text-[#0A0A0A]" />
                <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">Print</span>
              </button>
            </div>
          </div>
          ...
          <div className="bg-white w-[592px] rounded-modal border border-hw-border shadow-modal relative">
            ...
            <div className="p-10 flex flex-col gap-6 relative z-10">
                ...
                <div className="space-y-4">
                  <div className="flex gap-6">
                    ...
                    <div className="flex flex-col gap-1 items-end text-right">
                      <span className="text-xs font-medium text-hw-text-secondary tracking-[-0.12px]">Issue date:</span>
                      <Popover>
                        <PopoverTrigger asChild>
                          <button 
                            type="button"
                            className={cn(
                              "text-sm tracking-[-0.14px] hover:underline cursor-pointer text-right",
                              invoiceData.issueDate ? "text-hw-text" : "text-hw-inactive"
                            )}
                          >
                            {invoiceData.issueDate ? format(new Date(invoiceData.issueDate), "dd/MM/yyyy") : "DD/MM/YYYY"}
                          </button>
                        </PopoverTrigger>
                    ...
                    {/* Due Date - Editable */}
                    <div className="flex flex-col gap-1 items-end text-right">
                      <span className="text-xs font-medium text-hw-text-secondary tracking-[-0.12px]">Due date:</span>
                      <Popover>
                        <PopoverTrigger asChild>
                          <button 
                            type="button"
                            className={cn(
                              "text-sm tracking-[-0.14px] hover:underline cursor-pointer text-right",
                              invoiceData.dueDate ? "text-hw-text" : "text-hw-inactive"
                            )}
                          >
                            {invoiceData.dueDate ? format(new Date(invoiceData.dueDate), "dd/MM/yyyy") : "DD/MM/YYYY"}
                          </button>
                        </PopoverTrigger>
                        <PopoverContent className="w-auto p-0 z-50" align="end" sideOffset={4}>
                          <Calendar
                  ...
                  <div className="flex gap-6">
                    ...
                    <div className="flex flex-col gap-1 items-end text-right">
                      <span className="text-xs font-medium text-hw-text-secondary tracking-[-0.12px]">Invoice Number:</span>
                      <input
                        type="text"
                        value={`${invoiceData.invoiceNumberPrefix || "IV"}/${invoiceData.invoiceNumber.toString().padStart(4, "0")}`}
                        onChange={(e) => {
                          // Parse the input to extract prefix and number
                          const value = e.target.value;
                          const match = value.match(/^([A-Za-z]+)\/(\d+)$/);
                          if (match) {
                            setInvoiceData((prev) => ({
                        ...
                        className="text-sm text-hw-text tracking-[-0.14px] text-right bg-transparent border-none outline-none hover:underline focus:underline w-[80px]"
                      />
                    </div>
                    
                    {/* Reference - Editable */}
                    <div className="flex flex-col gap-1 items-end text-right">
                      <span className="text-xs font-medium text-hw-text-secondary tracking-[-0.12px]">Reference:</span>
                      <input
                        type="text"
                        value={invoiceData.reference || ""}
                        onChange={(e) => updateInvoiceField("reference", e.target.value)}
                        placeholder="Add reference..."
                        className="text-sm text-hw-text tracking-[-0.14px] text-right bg-transparent border-none outline-none hover:underline focus:underline w-[80px] placeholder:text-hw-inactive"
                      />
                    </div>
                  </div>
                </div>
              ...
              {/* Bill To */}
              <div className="space-y-2">
                <p className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Bill To:</p>
                <div className="space-y-0.5">
                  <p className="text-sm font-medium text-hw-text tracking-[-0.14px]">{invoiceData.name}</p>
                  <p className="text-sm text-hw-text tracking-[-0.14px]">{invoiceData.address}</p>
                </div>
              </div>

              {/* Invoice Title - Editable */}
              <div className="space-y-1.5">
                <p className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Invoice title</p>
                <div className="w-[190px]">
                  <input
                    ...
                    onChange={(e) => updateInvoiceField("title", e.target.value)}
                    placeholder="Enter invoice title..."
                    className="w-full h-8 px-2.5 py-1.5 text-sm font-medium text-hw-text tracking-[-0.14px] bg-white rounded shadow-[0px_0px_0px_1px_rgba(3,7,18,0.08),0px_0.5px_2px_0px_rgba(11,38,66,0.16)] focus:outline-none focus:ring-1 focus:ring-[#086DFF] placeholder:text-hw-inactive"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="px-6 space-y-2 relative z-10">
              {/* Jobs Heading */}
              <h3 className="text-base font-medium text-hw-text-secondary tracking-[-0.16px]">Jobs</h3>
              ...
              {levelOfDetail === "summary" && (() => {
                ...
                if (selectedJobs.length === 0) {
                  return (
                    <div className="text-center py-8 text-hw-text-secondary text-sm">
            ...
            <div className="px-6 pb-6 relative z-10">
              ...
              <div className="flex gap-10 pt-6">
                ...
                <div className="flex-1 space-y-6">
                  {/* Notes - Editable */}
                  <div className="space-y-1.5">
                    <p className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Notes</p>
                    <textarea
                      value={invoiceData.notes}
                      onChange={(e) => setInvoiceData((prev) => ({ ...prev, notes: e.target.value }))}
                      placeholder="Add notes..."
                ...
                <div className="w-[341px]">
                  ...
                  <div className="flex justify-between items-center py-3 border-t border-hw-border">
                    <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">Subtotal</span>
                    <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{formatCurrency(subtotal)}</span>
                  </div>
                  {/* VAT row */}
                  <div className="flex justify-between items-center py-3">
                    <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">VAT (Rate)</span>
                    <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{formatCurrency(vatAmount)}</span>
                  </div>
                  {/* Total row */}
                  <div className="flex justify-between items-center py-[10px] border-t border-hw-border">
                    <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">Total</span>
                    <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{formatCurrency(total)}</span>
                  </div>
                  {/* Amount due row */}
                  <div className="flex justify-between items-center py-3 border-t border-hw-border">
                    <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">Amount due</span>
                    <span className="text-xl font-bold text-hw-text tracking-[-0.2px]">{formatCurrency(total)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            <button
              className="p-1 rounded-md bg-white shadow-[0px_0px_0px_1px_rgba(11,38,66,0.08)] hover:bg-hw-surface-subtle transition-colors"
            ...
            <span className="text-base text-hw-text">
              ...
              <span className="text-hw-text-secondary"> of 2</span>
            </span>
            <button
              className="p-1 rounded-md bg-white shadow-[0px_0px_0px_1px_rgba(11,38,66,0.08)] hover:bg-hw-surface-subtle transition-colors"
