...
function InvoiceTitleBadge({ 
  ...
  if (isEditing) {
    return (
      <div className="inline-flex w-fit items-center gap-1.5 px-1.5 py-1 rounded-md bg-hw-brand/10 shadow-[0px_0px_0px_1px_rgba(3,7,18,0.08),0px_0.5px_2px_0px_rgba(11,38,66,0.16)]">
        <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">{invoiceNumber}</span>
        <FileText className="h-4 w-4 text-hw-text-secondary" />
        <input
          ref={inputRef}
          type="text"
          value={editValue}
          ...
          className="text-sm font-medium text-hw-text tracking-[-0.14px] bg-transparent border-none outline-none w-[120px]"
        />
      </div>
    );
  }
  ...
  return (
    <button
      onClick={handleClick}
      className="inline-flex w-fit items-center gap-1.5 px-1.5 py-1 rounded-md bg-hw-brand/10 shadow-[0px_0px_0px_1px_rgba(3,7,18,0.08),0px_0.5px_2px_0px_rgba(11,38,66,0.16)] hover:bg-hw-brand/[0.12] transition-colors cursor-pointer"
    >
      <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">{invoiceNumber}</span>
      <FileText className="h-4 w-4 text-hw-text-secondary" />
      <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{title}</span>
    </button>
  );
}
...
function DraftBadge() {
  return (
    <Badge variant="outline" className="px-1.5 py-0.5 bg-white border-hw-border text-xs font-medium text-hw-text tracking-[-0.12px]">
      Draft
    </Badge>
  );
}
...
function LinesBadge({
  ...
  if (isPartial && selected !== undefined) {
    return (
      <Badge variant="outline" className="px-1.5 py-0.5 rounded-md bg-hw-brand/10 border-hw-brand/20 text-sm font-medium text-hw-brand tracking-[-0.14px] w-fit">
        {selected} of {total} lines
      </Badge>
    );
  }
  if (isInactive) {
    return (
      <Badge variant="outline" className="px-1.5 py-0.5 rounded-md bg-hw-surface-subtle border-hw-border text-sm font-medium text-hw-text-secondary tracking-[-0.14px] w-fit">
        {total} lines
      </Badge>
    );
  }
  return (
    <Badge variant="outline" className="px-1.5 py-0.5 rounded-md bg-white border-hw-border text-sm font-medium text-hw-text-secondary tracking-[-0.14px] w-fit">
      {total} lines
    </Badge>
  );
}
...
function JobTypeBadge({ type }: { type: string }) {
  return (
    <Badge variant="outline" className="px-1.5 py-0.5 rounded-md bg-white border-hw-border text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">
      {type}
    </Badge>
  );
}
    ...
    <div className="flex flex-col gap-1.5 w-full">
      <div className="flex items-center gap-1">
        <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">
          Level of detail <span className="font-normal text-hw-text-secondary">(for all invoices)</span>
        </span>
        <HelpCircle className="h-4 w-4 text-hw-text-secondary" />
      </div>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button className="flex items-center justify-between w-full px-2 py-1.5 bg-white rounded-md shadow-[0px_0px_0px_1px_rgba(3,7,18,0.08),0px_0.5px_2px_0px_rgba(11,38,66,0.16)] hover:bg-hw-surface-hover transition-colors">
            <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{levelOfDetailLabels[value]}</span>
        ...
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          {levelOfDetailOptions.map((option) => (
            <button
              ...
              className={cn(
                "w-full flex items-center justify-between px-2 py-2 text-sm rounded hover:bg-hw-surface-subtle transition-colors text-left",
                value === option ? "bg-hw-surface-subtle text-hw-brand" : "text-hw-text"
              )}
            >
              <span>{levelOfDetailLabels[option]}</span>
              {value === option && <Check className="h-4 w-4 text-hw-brand" />}
            </button>
          ))}
        </PopoverContent>
      </Popover>
...
function GenericSelect({ 
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{label}</span>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button className="flex items-center justify-between w-full px-2 py-1.5 bg-white rounded-md shadow-[0px_0px_0px_1px_rgba(3,7,18,0.08),0px_0.5px_2px_0px_rgba(11,38,66,0.16)] hover:bg-hw-surface-hover transition-colors text-left">
            <span className="text-sm font-normal text-hw-text-secondary tracking-[-0.14px]">{selected.label}</span>
            <ChevronDown className="h-5 w-5 text-hw-text" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
    ...
    <div className="bg-white rounded-lg border border-hw-border overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <span className={cn(
          ...
          checked ? "text-hw-text" : "text-hw-inactive line-through"
        )}>
        ...
        {onGroupLinesSelectionChange && (
          <div className="bg-white rounded-lg border border-hw-border overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-3">
              ...
              <span className={cn(
                "text-sm font-bold tracking-[-0.14px]",
                groupLinesSelected ? "text-hw-text" : "text-hw-inactive"
              )}>
    ...
    <div className="bg-white rounded-lg border border-hw-border overflow-hidden">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-col gap-4 flex-1">
            ...
            <div className="space-y-0.5">
              <div className="flex items-center gap-2">
                <span className="text-sm font-bold text-hw-text tracking-[-0.14px]">{invoiceData.name}</span>
                <button className="text-sm font-normal text-hw-brand hover:underline tracking-[-0.14px]">Change</button>
              </div>
              <p className="text-sm font-normal text-hw-text-secondary tracking-[-0.14px] whitespace-pre-line">{invoiceData.address.split(", ").join(",\n")}</p>
            </div>
          </div>
              ...
              <div className="flex gap-6 items-start">
                <div className="flex flex-col gap-0.5 w-[125px] items-end">
                  <div className="flex items-center justify-end gap-1">
                    <Calendar className="h-4 w-4 text-hw-text-secondary" />
                    <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Issue date</span>
                  </div>
                  <span className="text-sm font-medium text-hw-text tracking-[-0.14px] text-right w-full">{formatDisplayDate(invoiceData.issueDate)}</span>
                </div>
                
                <div className="flex flex-col gap-0.5 w-[125px] items-end">
                  <div className="flex items-center justify-end gap-1">
                    <Calendar className="h-4 w-4 text-hw-text-secondary" />
                    <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Due date</span>
                  </div>
                  <span className="text-sm font-medium text-hw-text tracking-[-0.14px] text-right w-full">{formatDisplayDate(invoiceData.dueDate)}</span>
                </div>
              </div>
              
              <button className="p-0.5 hover:bg-hw-surface-hover rounded">
                <MoreVertical className="h-5 w-5 text-hw-text" />
              </button>
            </div>
              ...
              <div className="flex gap-6 items-start">
                <div className="flex flex-col gap-0.5 w-[112px] items-end">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Bank account</span>
                  </div>
                  <span className="text-sm font-medium text-hw-text tracking-[-0.14px] text-right w-full">{selected.label} ({selected.last4})</span>
                </div>
                
                <div className="flex flex-col gap-0.5 w-[112px] items-end">
                  <div className="flex items-center justify-end gap-1">
                    <FileText className="h-4 w-4 text-hw-text-secondary" />
                    <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Reference</span>
                  </div>
                  <span className="text-sm font-medium text-hw-text tracking-[-0.14px] text-right w-full">{invoiceData.reference || "Heating Upgrade"}</span>
                </div>
              </div>
      ...
      <div className="px-6 py-4">
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 py-2 rounded-md bg-hw-surface-subtle hover:bg-hw-surface-hover transition-colors text-hw-brand"
        >
          {jobsExpanded ? (
            <>
            ...
            return (
              <div key={job.id} className="bg-white rounded-lg border border-hw-border overflow-hidden">
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  <span className={cn(
                    "text-sm font-bold tracking-[-0.14px]",
                    isSelected ? "text-hw-text" : "text-hw-inactive line-through"
                  )}>
                    {formatCurrency(job.leftToInvoice)}
                  </span>
                </div>
      ...
      {/* Divider */}
      <div className="h-px bg-hw-border mx-6" />

      {/* Totals Section */}
      <div className="px-6 py-4 space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Subtotal</span>
          <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{formatCurrency(cardSubtotal)}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">VAT (Rate)</span>
          <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{formatCurrency(cardVat)}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">Total</span>
          <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{formatCurrency(cardSubtotal + cardVat)}</span>
        </div>
        <div className="flex items-center justify-between pt-2 border-t border-hw-border">
          <span className="text-sm font-bold text-hw-text tracking-[-0.14px]">Amount due</span>
          <span className="text-xl font-bold text-hw-text tracking-[-0.2px]">{formatCurrency(cardTotal)}</span>
        </div>
      </div>
    ...
    <div className="bg-white rounded-lg shadow-[0px_0px_0px_1px_rgba(26,28,46,0.12),0px_1px_2px_-1px_rgba(26,28,46,0.08),0px_2px_4px_0px_rgba(26,28,46,0.06)] overflow-hidden">
      ...
      <div className="px-5 pt-5 space-y-3">
        <div className="space-y-1">
          <h2 className="text-xl font-extrabold text-hw-text tracking-[-0.2px]">Overview</h2>
          <p className="text-sm font-medium text-hw-text-secondary tracking-[-0.14px]">{referenceNumber}</p>
        </div>
        <p className="text-sm font-bold text-hw-text tracking-[-0.14px]">{contactName}</p>
      </div>

      {/* Divider */}
      <div className="h-px bg-hw-border mx-0 my-4" />

      {/* Sites Section */}
      <div className="px-5 space-y-2">
        <p className="text-sm font-bold text-hw-text tracking-[-0.14px]">Sites</p>
        <div className="space-y-1">
          {sites.map((site, index) => (
            <p key={index} className="text-sm font-medium text-hw-text tracking-[-0.14px]">{site}</p>
          ))}
        </div>
      </div>
      ...
      <div className="px-5 pb-4 space-y-4">
        <p className="text-sm font-bold text-hw-text tracking-[-0.14px]">Settings</p>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">{structureLabels[breakdownLevel]}</span>
              <HelpCircle className="h-4 w-4 text-hw-text-secondary" />
            </div>
            <button 
              onClick={onBreakdownChange}
              className="text-sm font-normal text-hw-brand hover:underline tracking-[-0.14px]"
            >
              Change
            </button>
          </div>
      ...
      {/* Footer with Totals */}
      <div className="bg-hw-surface-subtle border-t border-hw-border px-5 py-4 space-y-4">
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">Sub-total</span>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">24%</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-hw-text tracking-[-0.14px]">Group total</span>
            <span className="text-base font-bold text-hw-text tracking-[-0.16px]">{formatCurrency(total)}</span>
          </div>
        </div>
        ...
        <div className="flex items-stretch w-full rounded-[var(--hw-radius-button,0.375rem)] shadow-[0_0_0_1px_rgba(7,98,229,0.8)] overflow-hidden">
          ...
          <button className="flex items-center justify-center px-2 bg-hw-brand hover:bg-hw-brand-hover border-l border-white/20">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1">
              <button 
                ...
                className="text-sm font-medium text-hw-text-secondary hover:text-hw-text tracking-[-0.14px]"
              >
