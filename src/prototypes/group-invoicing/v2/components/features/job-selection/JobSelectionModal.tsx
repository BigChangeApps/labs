...
function ProgressBar({ progress }: { progress: number }) {
  return (
    <div className="relative h-[8px] w-10 rounded-full bg-hw-surface-subtle overflow-hidden">
      <div
...
export function JobSelectionModal({ 
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[1157px] max-w-[1157px] max-h-[90vh] p-0 gap-0 overflow-hidden !rounded-modal border border-hw-border shadow-modal flex flex-col [&[data-state=open]]:duration-200 [&[data-state=open]]:animate-in [&[data-state=open]]:fade-in-0 [&[data-state=closed]]:duration-150 [&[data-state=closed]]:animate-out [&[data-state=closed]]:fade-out-0">
        {/* Header */}
        <div className="px-6 py-6 bg-hw-surface-subtle border-b border-hw-border flex items-center justify-between shrink-0">
          <h2 className="text-base font-semibold text-hw-text tracking-[-0.16px]">
            Select jobs
          </h2>
          <Button
            variant="secondary"
        ...
        <div className="px-4 py-6 border-b border-hw-border shrink-0">
          <div className="relative max-w-[260px]">
            ...
            <Input
              ...
              className="pl-8 h-7 text-sm bg-white border border-hw-border placeholder:text-hw-text-secondary"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col">
                ...
                <TableRow className="border-b border-hw-border hover:bg-transparent">
                  ...
                  <TableHead className="w-20 h-10 px-3">
                    <div className="flex items-center gap-1">
                      <span className="text-sm font-bold text-hw-text-secondary">Job</span>
                      <ChevronDown className="h-4 w-4 text-hw-text-secondary" />
                    </div>
                  </TableHead>
                  <TableHead className="h-10 px-3">
                    <span className="text-sm font-bold text-hw-text-secondary">Site</span>
                  </TableHead>
                  <TableHead className="h-10 px-3">
                    <span className="text-sm font-bold text-hw-text-secondary">Parent</span>
                  </TableHead>
                  <TableHead className="w-40 h-10 px-3">
                    <span className="text-sm font-bold text-hw-text-secondary">Completed</span>
                  </TableHead>
                  <TableHead className="w-20 h-10 px-3">
                    <span className="text-sm font-bold text-hw-text-secondary">Status</span>
                  </TableHead>
                  <TableHead className="w-20 h-10 px-3 text-right">
                    <span className="text-sm font-bold text-hw-text-secondary">Selling</span>
                  </TableHead>
                  <TableHead className="w-30 h-10 px-3 text-right">
                    <span className="text-sm font-bold text-hw-text-secondary">Left to invoice</span>
                  </TableHead>
                  ...
                  <TableHead className="w-10 h-10 px-3">
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-6 w-6 p-1 hover:bg-hw-surface-hover"
                    >
                      <ColumnsIcon />
                    </Button>
                  </TableHead>
                ...
                {paginatedJobs.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={11} className="h-32 text-center">
                      <p className="text-sm text-hw-text-secondary">No jobs found</p>
                    </TableCell>
                  </TableRow>
                ) : (
                    ...
                    return (
                      <TableRow
                        ...
                        className={cn(
                          ...
                          isSelected
                            ? "bg-hw-brand/8 hover:bg-hw-brand/12"
                            : "bg-white hover:bg-hw-surface-subtle"
                        )}
                      >
                          ...
                          <div className="flex items-center min-w-0">
                            <Badge
                              ...
                              className={cn(
                                ...
                                job.status === "Scheduled" && "bg-sky-100 border-sky-200 text-hw-text",
                                job.status === "In progress" && "bg-sky-600 border-sky-600 text-white hover:bg-sky-600",
                                (job.status === "Invoiced" || job.status === "Complete") && "bg-green-700 border-green-700 text-white hover:bg-green-700"
                              )}
                            >
                              {job.status}
                            </Badge>
          ...
          <div className="border-t border-hw-border bg-white px-4 py-3 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2 text-sm">
              <span className="text-hw-text">Items per page:</span>
              <select
                value={itemsPerPage}
                onChange={(e) => {
                ...
                className="text-sm font-medium text-hw-text border-none bg-transparent cursor-pointer"
              >
        ...
        <div className="px-6 py-4 border-t border-hw-border flex items-center justify-end bg-hw-surface-subtle shrink-0">
          <Button
            variant="default"
            size="default"
